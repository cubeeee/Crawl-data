...
import { API, ITEM_NAME, FacebookState } from './constant';

const Action: React.FC<InitalProps<FacebookState>> = (props) => {
  const { ids, setState } = props;
  ...
  const handleDelete = async () => {
    try {
      ...
      if (ids && ids.length > 0) {
        ...
        const successfulDeletes = deleteResults.filter(result =>  result.status === 'fulfilled' && result.value.data.status === 'success');
        message.open({
          key: 'action',
          type: 'success',
