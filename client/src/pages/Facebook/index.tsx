...
import { API, ITEM_NAME, FacebookState, SEARCH_COLUMNS } from './constant';
import CopyToClipboard from 'react-copy-to-clipboard';
import Export from './Export';

const initialState: InitalState<FacebookState> = {
  pagination: {
    current: 1,
    pageSize: PAGE_SIZE,
...
const Setting = () => {
  const [state, setState] = useState<InitalState<FacebookState>>(initialState);
  ...
  const TABLE_COLUMNS: ColumnsType<FacebookState> = [
  {
    title: 'Name',
    dataIndex: 'name',
    key: 'name',
  ...
  },
  {
    ...
    render: (value: string) => {
      return (
        <Tag color='cyan'>{value ? value : NaN}</Tag>
      )
    }
  },
  {
