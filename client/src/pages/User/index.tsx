...
import { Table, Row, Col, Space, Breadcrumb, Card, Button, Tooltip, message, Menu, Dropdown, Checkbox, Tag } from 'antd';
...
const initialState: InitalState<UserState> = {
  pagination: {
    current: 1,
    pageSize: PAGE_SIZE,
...
const Setting = () => {
  const [state, setState] = useState<InitalState<UserState>>(initialState);
  ...
  },
  {
    ...
    render: (value: string) => {
      return (
        <span>
          {
            value === 'Admin' ? <Tag color="#f50">{value}</Tag> :  <Tag color="#2db7f5">{value}</Tag> 
          }
