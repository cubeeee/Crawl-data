...
import { API_BASE_URL, SITE_NAME, VERSION } from "@configs/index";
  ...
  const checkLoginStatus = async () => {
    try {
      const res = await axios({
        method: "GET",
        url: `${API_BASE_URL}/api/auth/isLogin`,
        headers: {
    ...
    >
      <Layout>
        ...
        <Sider
          style={{
            padding: 0,
            background: themeSetting.isDark
              ? darkColor.colorBgContainer
              : themeSetting.isDarkMenu
              ? "#001529"
              : lightColor.colorBgContainer,
            position: "fixed",
          }}
        ...
        <Layout className="site-layout">
          <Header className="ant-fixed-header">
            ...
            {!open ? (
              <MenuUnfoldOutlined className="trigger" onClick={handleOpen} />
            ) : (
              <MenuFoldOutlined className="trigger" onClick={handleOpen} />
            )}
            <div className="ant-right-header">
              <UserHeader />
            </div>
          </Header>
          <span className="text-lg bg-red-500 ">hello</span>
          <Drawer
            className="ant-sidebar-mobile"
            style={{
              backgroundColor: themeSetting.isDark
                ? darkColor.colorBgContainer
                : themeSetting.isDarkMenu
                ? "#001529"
                : lightColor.colorBgContainer,
            ...
            >
              <Form layout="horizontal" labelCol={{ span: 7 }}>
                <Form.Item label="Theme">
                  ...
                  >
                    <Radio value="light">
                      <img alt="Light" src="./preview/light.svg" />
                      <Typography.Text>Light</Typography.Text>
                    </Radio>
                    <Radio value="dark">
                      <img alt="Dark" src="./preview/dark.svg" />
          ...
          <Footer className="center-aligned">
            {SITE_NAME} v{VERSION} ©{new Date().getFullYear()}
          </Footer>
        </Layout>
      </Layout>
    </ConfigProvider>
